...
@section('content')
        ...
        <div class="profile-card">
            <div class="profile-header">
                <div class="row">
                    ...
                    <div class="col-md-9">
                        <h2 class="mb-3">
                            <strong>{{ $admin->nama_lengkap ?? 'N/A' }}</strong>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-1"><strong>Kontak:</strong> {{ $admin->kontak ?? 'N/A' }}</p>
                                <p class="mb-1"><strong>Jenis Kelamin:</strong> {{ $admin->jenis_kelamin ?? 'N/A' }}</p>
                            </div>
                            <div class="col-md-6">
                                <p class="mb-1"><strong>Username:</strong> {{ $user->username ?? 'N/A' }}</p>
                                <p class="mb-1"><strong>Alamat:</strong> {{ $admin->alamat ?? 'N/A' }}</p>
                                <p class="mb-1"><strong>Status:</strong> {{ $admin->status ?? 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="editAdminForm" method="POST" action="{{ route('profileadmin.update', $admin->nip) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="jenis_kelamin" name="jenis_kelamin" required>
                                <option value="Laki Laki" {{ $admin->jenis_kelamin == 'Laki Laki' ? 'selected' : '' }}>Laki
                                    Laki</option>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="alamat" name="alamat" rows="3" required>{{ $admin->alamat }}</textarea>
                        </div>
                        <div class="mb-3">
                            <label for="kontak" class="form-label">Kontak</label>
                            <input type="text" class="form-control" id="kontak" name="kontak"
                                value="{{ $admin->kontak }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="status" class="form-label">Status</label>
                            <input type="text" class="form-control" id="status" name="status"
                                value="{{ $admin->status }}" required>
                        </div>
                    </div>
        ...
        cropButton.addEventListener('click', function() {
            if (croppieInstance) {
                ...
                croppieInstance.result({
                    ...
                    fetch("{{ route('profileadmin.updatePhoto', Auth::user()->admin->nip) }}", {
                            method: 'POST',
                            body: formData,
                            headers: {
